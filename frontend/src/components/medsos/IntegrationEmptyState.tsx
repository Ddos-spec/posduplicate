...
export function IntegrationEmptyState({ isDark, title, description, integration, onSetup }: Props) {
  return (
    <div className={`rounded-[32px] p-10 flex flex-col items-center text-center ${isDark ? 'bg-[#111318] ring-1 ring-white/10' : 'bg-white border-gray-100 shadow-sm'}`}>
      ...
      <button
        onClick={onSetup}
        className="inline-flex items-center gap-2 rounded-2xl bg-blue-600 hover:bg-blue-700 active:scale-95 transition-all text-white px-6 py-3 font-semibold"
      >
        <PlugZap size={18} />
        Setup {integration}
      </button>
