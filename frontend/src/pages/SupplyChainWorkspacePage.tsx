...
export default function SupplyChainWorkspacePage() {
  ...
  const resolveQC = async (qc: QualityCheck, status: 'pass' | 'fail' | 'waived') => {
    const answer = window.prompt(`Catatan QC (${status})`, qc.notes || '');
    if (answer === null) return;
    const notes = answer.trim();
    if ((status === 'fail' || status === 'waived') && !notes) return toast.error('Fail / waive wajib memiliki alasan');
    try {
      await resolveQualityCheck(qc.id, status, notes || undefined);
      await load();
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <main className="mx-auto max-w-[1700px] space-y-6 px-4 py-6 sm:px-6">
        ...
        {tab === 'manufacturing' && <div className="space-y-5"><section className={`rounded-2xl border p-4 ${card}`}><div className="mb-4"><h2 className="font-black">Create manufacturing order</h2><p className={`text-xs ${muted}`}>Recipe/BOM disnapshot saat MO dibuat supaya perubahan recipe berikutnya tidak mengubah histori produksi.</p></div><div className="grid gap-2 lg:grid-cols-4"><select className={input} value={moForm.outletId} onChange={(e) => setMoForm({ ...moForm, outletId: e.target.value, itemId: '' })}><option value="">Outlet</option>{outlets.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}</select><select className={input} value={moForm.itemId} onChange={(e) => setMoForm({ ...moForm, itemId: e.target.value })}><option value="">Finished product</option>{products.filter((p) => !moForm.outletId || p.outlet_id === Number(moForm.outletId)).map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><input className={input} type="number" min="0.001" step="0.001" value={moForm.quantityPlanned} onChange={(e) => setMoForm({ ...moForm, quantityPlanned: e.target.value })} placeholder="Planned qty" /><button onClick={() => void submitMO()} className="rounded-xl bg-cyan-600 px-4 py-2.5 text-sm font-bold text-white">Create MO</button></div></section><section className={`rounded-2xl border ${card}`}><div className="border-b border-inherit p-4"><h2 className="font-black">Manufacturing orders</h2></div><div className="divide-y divide-inherit">{manufacturingOrders.map((mo) => <article key={mo.id} className="p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex gap-2"><p className="font-black">{mo.mo_number}</p><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}>{mo.status}</span></div><p className={`mt-1 text-xs ${muted}`}>{mo.item_name || `Item ${mo.item_id}`} · plan {number(mo.quantity_planned)} · produced {number(mo.quantity_produced)} · {mo.consumptions.length} BOM lines</p><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><div className={`rounded-lg border px-2.5 py-2 ${isDark ? 'border-slate-700' : 'border-slate-200'}`}><p className={`text-[10px] uppercase ${muted}`}>Yield</p><p className="text-sm font-black">{number(mo.yield_percentage ?? mo.yieldPercentage ?? 0)}%</p></div><div className={`rounded-lg border px-2.5 py-2 ${isDark ? 'border-slate-700' : 'border-slate-200'}`}><p className={`text-[10px] uppercase ${muted}`}>Planned material</p><p className="text-sm font-black">{money(mo.planned_material_cost ?? mo.plannedMaterialCost ?? 0)}</p></div><div className={`rounded-lg border px-2.5 py-2 ${isDark ? 'border-slate-700' : 'border-slate-200'}`}><p className={`text-[10px] uppercase ${muted}`}>Consumed cost</p><p className="text-sm font-black">{money(mo.consumed_material_cost ?? mo.consumedMaterialCost ?? 0)}</p></div><div className={`rounded-lg border px-2.5 py-2 ${isDark ? 'border-slate-700' : 'border-slate-200'}`}><p className={`text-[10px] uppercase ${muted}`}>Output / unit</p><p className="text-sm font-black">{money(mo.output_unit_cost ?? mo.outputUnitCost ?? 0)}</p></div></div></div><div className="flex flex-wrap gap-2">{mo.status === 'draft' && <button onClick={() => void moAction(mo, 'confirm')} className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-bold text-white">Confirm</button>}{mo.status === 'confirmed' && <button onClick={() => void moAction(mo, 'start')} className="rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-bold text-white">Start</button>}{mo.status === 'in_progress' && <button onClick={() => void completeMO(mo)} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white">Complete</button>}{['draft', 'confirmed'].includes(mo.status) && <button onClick={() => void moAction(mo, 'cancel')} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>Cancel</button>}</div></div></article>)}</div></section></div>}

        {tab === 'quality' && <section className={`rounded-2xl border ${card}`}><div className="border-b border-inherit p-4"><h2 className="font-black">Quality control queue</h2><p className={`mt-1 text-xs ${muted}`}>MO completion automatically creates a pending production-output QC record.</p></div><div className="divide-y divide-inherit">{qualityChecks.length === 0 ? <p className={`p-6 ${muted}`}>Belum ada QC.</p> : qualityChecks.map((qc) => <article key={qc.id} className="p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex gap-2"><p className="font-bold">{qc.check_type}</p><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${qc.status === 'pass' ? 'bg-emerald-500/15 text-emerald-500' : qc.status === 'fail' ? 'bg-rose-500/15 text-rose-500' : 'bg-amber-500/15 text-amber-500'}`}>{qc.status}</span></div><p className={`mt-1 text-xs ${muted}`}>{qc.item_name || qc.inventory_name || 'General check'} · {qc.reference_type || 'manual'} {qc.reference_id || ''}</p>{qc.check_type === 'production_output' && qc.criteria && <div className={`mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] ${muted}`}><span>Output {number(qc.criteria.quantityProduced)}</span><span>Yield {number(qc.criteria.yieldPercentage)}%</span><span>Material {money(qc.criteria.materialCost)}</span><span>Unit cost {money(qc.criteria.outputUnitCost)}</span></div>}</div>{qc.status === 'pending' && <div className="flex gap-2"><button onClick={() => void resolveQC(qc, 'pass')} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white">Pass</button><button onClick={() => void resolveQC(qc, 'fail')} className="rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-bold text-white">Fail</button><button onClick={() => void resolveQC(qc, 'waived')} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>Waive</button></div>}</div></article>)}</div></section>}
