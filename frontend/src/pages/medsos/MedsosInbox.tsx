...
function ZernioConversationPanel({ isDark }: { isDark: boolean }) {
  ...
  useEffect(() => {
    ...
    getZernioConversations({
      ...
      .finally(() => setLoading(false));
  }, [refreshTick, platform, accountId]);
  ...
  const availableAccounts = useMemo(() => {
    ...
    for (const conversation of conversations) {
      map.set(conversation.accountId, conversation.accountUsername || conversation.accountId);
    }
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <select
              ...
              className={`flex-1 text-xs rounded-xl border-0 ring-1 ring-inset ring-gray-200 dark:ring-white/10 px-3 py-2 outline-none ${isDark ? 'bg-[#111318] ring-1 ring-white/10 text-white' : 'bg-white border-gray-200 text-gray-700 shadow-sm'}`}
            >
      ...
      <div className={`flex-1 flex flex-col min-w-0 relative ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
        {!selected ? (
          ...
          <>
            ...
            <div className={`p-6 border-t ${isDark ? 'border-slate-700 bg-slate-900' : 'border-gray-100 bg-white'}`}>
              <div className={`relative flex items-end gap-3 p-2 rounded-2xl border-2 transition-all focus-within:ring-4 ${
                ...
                <button
                  ...
                  onClick={() => toast('Lampiran untuk inbox live masih menunggu endpoint media provider.')}
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    aria-label="Buat balasan AI"
                    title="Buat balasan AI"
            ...
            <button
              type="button"
              aria-label="Buat draft AI"
              title="Buat draft AI"
              onClick={() => void handleAiReply()}
              disabled={sending || !selected}
              className="absolute bottom-32 right-10 h-14 w-14 rounded-full bg-orange-600 text-white shadow-2xl shadow-orange-600/40 flex items-center justify-center transition-transform hover:scale-110 active:scale-90 z-20 disabled:opacity-50"
            >
      ...
      <main className={`flex flex-col min-w-0 ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
        ...
        <div className={`p-4 border-t ${isDark ? 'bg-[#111318] ring-1 ring-white/10' : 'bg-white ring-1 ring-slate-900/5'}`}>
          ...
          <div className={`flex items-center gap-2 rounded-2xl border px-2 py-2 ${isDark ? 'bg-slate-700 border-slate-600' : 'bg-gray-50 border-gray-200'}`}>
            <button type="button" aria-label="Lampirkan file" title="Lampirkan file" onClick={() => toast('Lampiran demo belum aktif.')} className="p-2 text-gray-400 hover:text-gray-600">
              <Paperclip size={20} />
            </button>
            <input
              type="text"
