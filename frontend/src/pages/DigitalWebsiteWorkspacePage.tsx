...
export default function DigitalWebsiteWorkspacePage() {
  ...
  return <div className="min-h-screen bg-slate-50 p-6 text-slate-900">
    <div className="mx-auto max-w-6xl">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold text-blue-600">P3 DIGITAL BUSINESS</p><h1 className="text-3xl font-black">Website & Storefront</h1><p className="mt-2 text-slate-600">CMS, publication workflow, dan catalog projection tanpa product master paralel.</p></div>{selected?.status === 'published' && <button onClick={() => window.open(`/store/${selected.public_slug}`, '_blank', 'noopener,noreferrer')} className="rounded-lg border bg-white px-3 py-2 text-sm font-bold"><ExternalLink className="inline h-4 w-4" /> Public preview</button>}</div>
