...
const MOCK_INGREDIENTS: Ingredient[] = [
  { id: 1, name: 'Beras', unit: 'kg', cost_per_unit: 12000 },
  { id: 2, name: 'Telur', unit: 'butir', cost_per_unit: 2000 },
  { id: 3, name: 'Gula Pasir', unit: 'kg', cost_per_unit: 15000 },
  { id: 4, name: 'Kopi Arabika', unit: 'gram', cost_per_unit: 250 },
  { id: 5, name: 'Susu Fresh Milk', unit: 'liter', cost_per_unit: 18000 },
];

const MOCK_EXISTING_RECIPES: Record<number, RecipeItem[]> = {
  1: [ // Nasi Goreng
    { ingredient_id: 1, ingredientName: 'Beras', quantity: 0.15, unit: 'kg' },
    { ingredient_id: 2, ingredientName: 'Telur', quantity: 1, unit: 'butir' },
  ],
  4: [ // Kopi Susu
    { ingredient_id: 4, ingredientName: 'Kopi Arabika', quantity: 18, unit: 'gram' },
    { ingredient_id: 5, ingredientName: 'Susu Fresh Milk', quantity: 0.15, unit: 'liter' },
  ]
};

export default function DemoRecipeModal({ product, onClose }: RecipeModalProps) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        setRecipeItems(MOCK_EXISTING_RECIPES[product.id] || []);
      } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      toast.success('Recipe saved successfully (Demo)');
      onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="p-4 border-b flex justify-between items-center">
          <h3 className="text-xl font-bold">Resep untuk {product.name} (Demo)</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="text-center py-8 text-gray-500">Loading recipe data...</div>
          ) : (
