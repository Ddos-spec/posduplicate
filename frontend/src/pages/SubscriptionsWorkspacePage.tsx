...
export default function SubscriptionsWorkspacePage() {
  ...
  const renew = async (id: number) => {
    setBusy(true); setError('');
    try { await renewCustomerSubscription(id); await reload(); } catch { setError('Renewal gagal. Kontrak harus aktif dan downstream sales/receivable harus valid.'); } finally { setBusy(false); }
  };
