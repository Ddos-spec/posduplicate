...
import { ArrowLeft, Clock3, LogIn, LogOut, RefreshCw, Users } from 'lucide-react';
...
export default function WorkforceWorkspacePage() {
  ...
  const todaySessions = useMemo(() => {
    ...
    return sessions.filter((row) => new Date(row.clock_in_at).toDateString() === today);
  }, [sessions]);
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.error?.message || 'Clock-in gagal');
  ...
  const handleClockOut = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.error?.message || 'Clock-out gagal');
    } finally { setWorking(false); }
  };
      ...
      <header className="border-b border-slate-800 bg-slate-950/95 px-4 py-4 md:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-400">P2 Workforce</p><h1 className="text-xl font-black">Employees & Attendance</h1></div>
          </div>
          <button onClick={() => void load()} disabled={loading} className="rounded-xl border border-slate-700 p-2 hover:bg-slate-800"><RefreshCw size={18} className={loading ? 'animate-spin' : ''} /></button>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
        <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
            ...
            ) : <div className="mt-5 rounded-xl border border-amber-700/40 bg-amber-950/20 p-4 text-sm text-amber-200">Akun login belum terhubung ke employee aktif. Owner/HR perlu menghubungkan employee ke user account.</div>}
          </div>
