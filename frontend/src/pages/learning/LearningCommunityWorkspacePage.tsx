...
export default function LearningCommunityWorkspacePage() {
  ...
  const createTopic = async (event: FormEvent) => {
    ...
    try {
      ...
      setTopicForm({ title: '', slug: '', body: '' }); toast.success('Topic dibuat'); await loadForum(selectedForum);
    } catch (error: any) { toast.error(error?.response?.data?.error?.message || 'Topic gagal dibuat'); }
  };

  const createReply = async (event: FormEvent) => {
    event.preventDefault(); if (!selectedTopic || !replyBody.trim()) return;
    try { await createCommunityReply(selectedTopic.id, { content: paragraph(replyBody) }); setReplyBody(''); toast.success('Reply dibuat'); await loadTopic(selectedTopic); }
    ...
    <div className="mx-auto max-w-7xl space-y-5">
      ...
      {tab === 'learning' ? <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <div>{selectedCourse ? <div className="space-y-4">
          <section className="rounded-2xl border bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">{selectedCourse.title}</h2><p className="text-sm text-slate-500">{selectedCourse.slug} · {selectedCourse.visibility} · {selectedCourse.status}</p></div>{selectedCourse.status !== 'archived' && <button onClick={async () => { try { await setLearningCourseStatus(selectedCourse.id, selectedCourse.status === 'published' ? 'draft' : 'published'); await loadRoot(); await loadCourse(selectedCourse); } catch (e:any) { toast.error(e?.response?.data?.error?.message || 'Status gagal'); } }} className="rounded-xl border px-3 py-2 text-sm font-bold">{selectedCourse.status === 'published' ? 'Unpublish' : 'Publish course'}</button>}</div></section>
        ...
        <div>{selectedForum ? <div className="space-y-4"><section className="rounded-2xl border bg-white p-5"><div className="flex items-center justify-between"><div><h2 className="text-xl font-black">{selectedForum.name}</h2><p className="text-sm text-slate-500">{selectedForum.status} · {selectedForum.visibility}</p></div>{selectedForum.status !== 'archived' && <button onClick={async () => { try { await setCommunityForumStatus(selectedForum.id, selectedForum.status === 'published' ? 'draft' : 'published'); await loadRoot(); await loadForum(selectedForum); } catch (e:any) { toast.error(e?.response?.data?.error?.message || 'Status gagal'); } }} className="rounded-xl border px-3 py-2 text-sm font-bold">{selectedForum.status === 'published' ? 'Unpublish' : 'Publish forum'}</button>}</div></section><form onSubmit={createTopic} className="space-y-2 rounded-2xl border bg-white p-4"><h3 className="font-bold">Create staff topic</h3><input required value={topicForm.title} onChange={(e) => setTopicForm({ ...topicForm, title: e.target.value })} placeholder="Topic title" className="w-full rounded-xl border px-3 py-2"/><textarea required value={topicForm.body} onChange={(e) => setTopicForm({ ...topicForm, body: e.target.value })} placeholder="Topic body" className="w-full rounded-xl border px-3 py-2"/><button className="rounded-xl bg-slate-900 px-4 py-2 font-bold text-white">Create topic</button></form><div className="grid gap-4 lg:grid-cols-2"><section className="rounded-2xl border bg-white p-4"><h3 className="font-bold">Topics</h3><div className="mt-3 space-y-2">{topics.map((topic) => <div key={topic.id} className={`rounded-xl border p-3 ${selectedTopic?.id === topic.id ? 'border-blue-500' : ''}`}><button onClick={() => void loadTopic(topic)} className="w-full text-left"><div className="font-semibold">{topic.pinned ? '📌 ' : ''}{topic.title}</div><div className="text-xs text-slate-500">{topic.status} · {topic.reply_count || 0} replies</div></button><div className="mt-2 flex gap-3 text-xs font-bold"><button onClick={async () => { await moderateCommunityTopic(topic.id,{pinned:!topic.pinned}); await loadForum(selectedForum); }} className="text-blue-700">{topic.pinned?'Unpin':'Pin'}</button><button onClick={async () => { await moderateCommunityTopic(topic.id,{status:topic.status==='locked'?'open':'locked'}); await loadForum(selectedForum); }} className="text-amber-700">{topic.status==='locked'?'Open':'Lock'}</button><button onClick={async () => { await moderateCommunityTopic(topic.id,{status:'hidden'}); await loadForum(selectedForum); }} className="text-rose-700">Hide</button></div></div>)}</div></section><section className="rounded-2xl border bg-white p-4"><h3 className="font-bold">Replies</h3>{selectedTopic ? <><form onSubmit={createReply} className="mt-3 flex gap-2"><input required value={replyBody} onChange={(e) => setReplyBody(e.target.value)} placeholder="Staff reply" className="flex-1 rounded-xl border px-3 py-2"/><button className="rounded-xl bg-slate-900 px-3 py-2 font-bold text-white">Reply</button></form><div className="mt-3 space-y-2">{replies.map((reply) => <div key={reply.id} className="rounded-xl bg-slate-50 p-3"><div className="text-xs font-bold">{reply.author_name}</div><div className="mt-1 text-sm">{reply.content?.[0]?.text || 'Reply'}</div><div className="mt-2 flex gap-2 text-xs font-bold"><button onClick={async () => { await moderateCommunityReply(reply.id,'hidden'); await loadTopic(selectedTopic); }} className="text-rose-700">Hide</button><button onClick={async () => { await moderateCommunityReply(reply.id,'visible'); await loadTopic(selectedTopic); }} className="text-blue-700">Restore</button></div></div>)}</div></> : <p className="mt-4 text-sm text-slate-500">Select a topic.</p>}</section></div></div> : <div className="rounded-2xl border bg-white p-10 text-center text-slate-500"><MessageSquareText className="mx-auto mb-3"/>Select a forum</div>}</div>
      </div>}
