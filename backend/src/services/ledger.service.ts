...
export const postJournalToLedger = async (journalId: number, tenantId: number, userId: number) => {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const currentJournal = await tx.journal_entries.findUnique({ where: { id: journal.id } });
    if (!currentJournal || currentJournal.tenant_id !== tenantId) throw new Error('Journal not found');
    if (currentJournal.status !== 'draft') throw new Error('Only draft journals can be posted');
