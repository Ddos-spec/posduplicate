...
const resolveMigrationFile = (migrationName: string) => {
  ...
  if (!found) {
    throw new Error(`Migration SQL not found for ${migrationName}. Checked: ${candidates.join(', ')}`);
  }
  return found;
};
...
const ensureMigrationLedger = async (client: Client) => {
  await client.query(`
    CREATE TABLE IF NOT EXISTS public.suite_schema_migrations (
...
const applyMigration = async (client: Client, migrationName: string) => {
  const migrationFile = resolveMigrationFile(migrationName);
  const sql = fs.readFileSync(migrationFile, 'utf8');
  const checksum = sha256(sql);

  const existing = await client.query<{ checksum_sha256: string }>(
    'SELECT checksum_sha256 FROM public.suite_schema_migrations WHERE migration_name = $1 LIMIT 1',
    [migrationName]
  );
  ...
  try {
    ...
    await client.query(
      'INSERT INTO public.suite_schema_migrations (migration_name, checksum_sha256) VALUES ($1, $2)',
...
const run = async () => {
  ...
  try {
    ...
    const applied = await client.query<{ migration_name: string; applied_at: Date }>(
      'SELECT migration_name, applied_at FROM public.suite_schema_migrations WHERE migration_name = ANY($1::text[]) ORDER BY migration_name',
      [MIGRATIONS]
    );
